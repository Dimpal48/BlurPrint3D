...
import {getAllServicesItems, saveItem} from "../utils/firebaseFunctions"
...
function CreateServices() {
  ...
  const uploadImage = (e) => {
    ...
    const storageRef = ref(
      storage,
      `ServicesImages/${Date.now()}-${imageFile.name}`
    );
  ...
  const saveDetails = () => {
    ...
    try {
      if ((!title  || !imageAsset || !desc || !category)) {
        ...
        // saveItem(data);
        const dbRef=collection(firestore,"ServicesItems")
        addDoc(dbRef,data)
        setIsLoading(false);
        setFields(true);
  ...
  const clearData = () => {
    ...
    const fetchData=async()=>{
      await getAllServicesItems().then((data)=>{
        dispatch({
          type : actionType.SET_SERVICES_ITEMS,
          servicesItems:data,
        })
      })
    }
     ...
     <div
      ...
      >
          ...
          <motion.p
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={`w-96 p-2 h-20 mt-2  rounded-md text-center text-white font-semibold ${
              alertStatus === "danger"
